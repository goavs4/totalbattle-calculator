...
function buildExcludeToggle(squad) {
  ...
  section.innerHTML = `
    <div class="exclude-box">
      ...
      ${analysis.warnings.map(w=>`<div class="warning-line ${w.severity.toLowerCase()}">${w.message}</div>`).join("")}
      ...
      <div id="excludePreview" class="exclude-preview" style="display:none">
        Will remove from ALL pools: <strong>${[...affTypes,...affSubtypes].join(", ")}</strong>
      </div>
    </div>`;
...
// up to 3 unit types (ranged, melee, mounted) respecting
// weights and 2x mounted leadership cost
// ============================================================
function allocate3(budget, rw, mw, cw) {
  const totalW = rw + mw + (cw*2);
  if (totalW <= 0) return { rangedN:0, meleeN:0, mountedN:0 };
  const upw    = budget / totalW;
  let rangedN  = Math.floor(upw * rw);
  let meleeN   = Math.floor(upw * mw);
  let mountedN = Math.floor((upw * cw * 2) / 2);
  // Rounding remainder → ranged (or melee if ranged excluded)
  const used = rangedN + meleeN + (mountedN*2);
  if (rw > 0)       rangedN  += budget - used;
  else if (mw > 0)  meleeN   += budget - used;
  else               mountedN += Math.floor((budget-used)/2);
  return { rangedN, meleeN, mountedN };
}
...
function calculate() {
  ...
  if (leadership > 0) {
    ...
    // GUARDSMEN
    if (hasG && gL > 0) {
      const rw = isExcluded(excl,"Ranged","Human")  ? 0 : 1.0;
      const mw = isExcluded(excl,"Melee","Human")   ? 0 : 1.0;
      const cw = isExcluded(excl,"Mounted","Human") ? 0 : 1.0;
      const fw = isExcluded(excl,"Flying","Beast")  ? 0 : 1.0;
      const totalW = rw + mw + (cw*2);
      ...
      if (totalW <= 0) {
        ...
        const { rangedN, meleeN, mountedN } = allocate3(gL, rw, mw, cw);
        if (rangedN > 0) {
          out += `🏹 Archers [Ranged, Human] — ${rangedN.toLocaleString()}\n`;
          out += tierLines(splitTroops(rangedN, tiersG, "G"));
        }
        if (meleeN > 0) {
          out += `🗡️  Spearmen [Melee, Human] — ${meleeN.toLocaleString()}\n`;
          out += tierLines(splitTroops(meleeN, tiersG, "G"));
        }
        if (mountedN > 0) {
          out += `🐎 Riders [Mounted, Human] — ${mountedN.toLocaleString()} (×2 leadership)\n`;
    ...
    if (hasS && sL > 0) {
      ...
      if (totalW <= 0) {
        ...
        const { rangedN, meleeN, mountedN } = allocate3(sL, rw, mw, cw);
        if (rangedN > 0) {
          out += `🏹 Vultures/Deadshot [Ranged, Human] — ${rangedN.toLocaleString()}\n`;
          out += tierLines(splitTroops(rangedN, tiersS, "S"));
        }
        if (meleeN > 0) {
          out += `🗡️  Swordsmen [Melee, Human] — ${meleeN.toLocaleString()}\n`;
          out += tierLines(splitTroops(meleeN, tiersS, "S"));
        }
        if (mountedN > 0) {
          out += `🐎 Lion Riders [Mounted, Human] — ${mountedN.toLocaleString()} (×2 leadership)\n`;
...
function renderMercsTab(c) {
  c.innerHTML = `<div class="modal-section">
    ...
    <p class="hint">Add each mercenary from the Mercenary Exchange. Examples: Arbalaster VII (Ranged, Guardsman), Golden Dragon VII (Flying, Dragon), Jungle King (Melee, Beast).</p>
    ${buildUnitForm("merc")}
    ${DB.local.mercs.length > 0
      ? `<div class="unit-list">${DB.local.mercs.map((m,i)=>`
          <div class="unit-row">
            ...
            <button class="btn-delete" onclick="deleteUnit('mercs',${i})">✕</button>
          </div>`).join("")}</div>`
      : `<p class="hint empty-hint">No mercenaries added yet.</p>`}
...
function renderMonstersTab(c) {
  c.innerHTML = `<div class="modal-section">
    ...
    ${DB.local.monsters.length > 0
      ? `<div class="unit-list">${DB.local.monsters.map((m,i)=>`
          <div class="unit-row">
            <span class="unit-name">${m.name}</span>
            <span class="unit-tags">${m.combat_type} · ${m.subtype} · T${m.tier||"?"}</span>
            <span class="unit-stats">STR:${m.strength||"?"} HP:${m.health||"?"} Cost:${m.resource_cost||"?"}</span>
            <button class="btn-delete" onclick="deleteUnit('monsters',${i})">✕</button>
          </div>`).join("")}</div>`
      : `<p class="hint empty-hint">No player monsters added yet.</p>`}
...
function renderEnemiesTab(c) {
  ...
  let html = `<div class="modal-section">
    ...
    <p class="hint">Add squads you encounter. Bonus format: TypeName:value e.g. <code>Melee:35,Mounted:20</code></p>
    <div class="form-group-row">
      ...
      <select id="ns-cat" class="form-input">${cats.map(c=>`<option value="${c}">${formatLabel(c)}</option>`).join("")}</select>
      <select id="ns-fac" class="form-input">${facs.map(f=>`<option value="${f}">${formatLabel(f)}</option>`).join("")}</select>
  ...
  DB.local.enemies.forEach((sq, si) => {
    ...
    (sq.units||[]).forEach((u,ui)=>{
      ...
      html += `<div class="unit-row sub">
        <span class="unit-name">${u.unit_name} ×${u.count||"?"}</span>
    ...
    html += `<div class="form-group-row sub-form">
      ...
      <select id="eu-type-${si}" class="form-input short">${COMBAT_TYPES.map(t=>`<option>${t}</option>`).join("")}</select>
      <select id="eu-sub-${si}"  class="form-input short">${SUBTYPES.map(s=>`<option>${s}</option>`).join("")}</select>
...
function buildUnitForm(type) {
  ...
  return `<div class="form-group-row">
    ...
    <select id="${id}-type" class="form-input short">${COMBAT_TYPES.map(t=>`<option>${t}</option>`).join("")}</select>
    <select id="${id}-sub"  class="form-input short">${SUBTYPES.map(s=>`<option>${s}</option>`).join("")}</select>
